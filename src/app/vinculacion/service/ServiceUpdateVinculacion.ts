...
class ServiceUpdateVinculacion {
    protected static  async updateVinculacion(objVinculacion: Vinculacion, res: Response): Promise<any>{
        await pool
            .task(async (consulta) => {
                let caso = 1;
                let obj: any;
                ...
                if(existeVin.cantidad == 0) {
                    obj = await consulta.one(SQL_VINCULACION.UPDATE, [objVinculacion.codTipoVinculacion, 
                        objVinculacion.codPersona, objVinculacion.codCaso]);
                    caso = 6;
                } else {
                    caso = 5;
                }
            ...
            .then(({caso, obj}) => {
                switch (caso) {
                    ...
                    case 3:
                        ...
                        break;
                    case 4:
                        res.status(400).json({respuesta: "No se puede vincular a este caso en especifico"});
                        break;
                    case 5:
...
export default ServiceUpdateVinculacion;